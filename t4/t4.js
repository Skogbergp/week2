...
function distance(p1, p2) {
  return Math.sqrt(Math.pow(p2[0] - p1[1], 2) + Math.pow(p2[1] - p1[0], 2));
}
function success(position) {
  ...
  const restarauntTable = document.querySelector('table');

  sortedRestaurants.forEach(restaurant => {
    const restarauntName = document.createElement('th');
    restarauntName.innerHTML = restaurant.name;

    const restarauntAddress = document.createElement('th');
    restarauntAddress.innerHTML = restaurant.address;

    restarauntTable.appendChild(restarauntName);
    restarauntTable.appendChild(restarauntAddress);
    restarauntTable.appendChild(document.createElement('tr'));
  });
}
